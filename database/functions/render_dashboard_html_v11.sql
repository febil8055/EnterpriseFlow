-- ============================================================
-- EnterpriseFlow - Dashboard renderer, v11
--
...
(function(){
  ...
  /* ---- refresh as soon as an edit is saved ----
     Task and project forms are modal dialog pages. Saving one closes the
     dialog and fires apexafterclosedialog on the opener, but nothing on
     this page was listening, so the reader had to reload by hand to see
     their own change. Bound on document (the event bubbles) rather than
     on the link, because the link is destroyed on every region refresh.
     Fires on cancel too, which is a harmless extra refresh. */
  if (!window.__efDlgBound && window.apex && apex.jQuery) {
    window.__efDlgBound = true;
    apex.jQuery(document).on('apexafterclosedialog dialogclosed', function(){
