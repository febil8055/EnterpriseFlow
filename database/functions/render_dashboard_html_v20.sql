-- ============================================================
-- EnterpriseFlow - Dashboard renderer, v20
--
...
CREATE OR REPLACE FUNCTION "RENDER_DASHBOARD_HTML" RETURN CLOB IS
  ...
  l_health NUMBER; l_health_raw NUMBER; l_health_label VARCHAR2(30);
  ...
  -- Unrounded. Deltas must be measured on this, not on the displayed
  -- integer: with 54 open tasks a single action moves the score about
  -- 0.6 points, which rounds to zero. The recommendation panel was
  -- gated on "delta > 0", so as the workspace grew the most valuable
  -- element on the page silently stopped rendering.
  FUNCTION health_raw(p_open IN NUMBER, p_overdue IN NUMBER,
  ...
  FUNCTION health_of(p_open IN NUMBER, p_overdue IN NUMBER,
                     p_blocked IN NUMBER, p_unassigned IN NUMBER) RETURN NUMBER IS
  BEGIN
    RETURN ROUND(health_raw(p_open, p_overdue, p_blocked, p_unassigned));
  END health_of;

  -- One decimal, NLS-safe, no trailing ".0" on whole numbers.
  FUNCTION dec1(p_n IN NUMBER) RETURN VARCHAR2 IS
    l_s VARCHAR2(40) := TRIM(TO_CHAR(ROUND(p_n,1), 'FM99999990.0',
                                     'NLS_NUMERIC_CHARACTERS=''.,'''));
  BEGIN
    RETURN CASE WHEN SUBSTR(l_s,-2) = '.0' THEN SUBSTR(l_s,1,LENGTH(l_s)-2) ELSE l_s END;
  END dec1;
...
BEGIN
  ...
  l_health_raw := health_raw(l_tasks_open, l_overdue, l_blocked, l_unassigned);
  l_health     := ROUND(l_health_raw);
  ...
  --------------------------------------------------------------------
  IF l_unassigned > 0 THEN
    l_d_assign := health_raw(l_tasks_open, l_overdue, l_blocked, l_unassigned - 1) - l_health_raw;
  END IF;
  IF l_blocked > 0 THEN
    l_d_unblock := health_raw(l_tasks_open, l_overdue, l_blocked - 1, l_unassigned) - l_health_raw;
  END IF;
  IF l_overdue > 0 THEN
    l_d_resolve := health_raw(l_tasks_open - 1, l_overdue - 1, l_blocked, l_unassigned) - l_health_raw;
  END IF;
  ...
  IF l_rec_kind IS NOT NULL THEN
    l_rec_after_raw := l_health_raw + l_rec_delta;
    l_rec_after     := ROUND(l_rec_after_raw);
  END IF;
  ...
  DECLARE
    l_cached_fp VARCHAR2(200); l_cached_txt VARCHAR2(1000); l_cached_at DATE;
  ...
  BEGIN
    ...
    IF NOT l_fresh THEN
      ...
      l_prompt :=
        ...
        ||'Open tasks with no owner: '||l_unassigned||CHR(10)
        ...
        ||CHR(10)
        ||'Brief an executive in exactly two sentences, 30 words in total at the very most.'||CHR(10)
        ||'Sentence 1: the single most important thing to act on, and why it matters.'||CHR(10)
        ||'Sentence 2: one short line of overall context.'||CHR(10)
        ||'Do not list or recite the figures above - mention at most three numbers in total.'||CHR(10)
        ||'Keep every figure attached to its own label: never describe a total as "active",'
        ||' and never state a number that is not listed above.'||CHR(10)
        ||'Plain text only, no markdown, no greeting.'||CHR(10)
        -- The example uses invented names and numbers on purpose. An
        -- earlier version illustrated the format with the real current
        -- figures, and the model simply echoed it back word for word -
        -- which reads fine today and becomes a fabricated brief the
        -- moment the data moves. Nothing here can be copied as fact.
        ||'Match the shape and length of this example, but use the real'
        ||' figures above - the example itself is invented:'||CHR(10)
        ||'"Vendor contract review is 4 days overdue and is holding up Orion Migration.'
        ||' The other 7 active projects are tracking normally."';
      ...
      IF l_raw IS NOT NULL AND LENGTH(l_raw) > 0 THEN
        l_insight := SUBSTR(l_raw, 1, 400);
        -- A hard SUBSTR left the card ending mid-word when the model
        -- overran. Fall back to the last complete sentence instead;
        -- the >80 guard stops a stray early period truncating the
        -- whole brief down to a fragment.
        IF LENGTH(l_raw) > 400 AND INSTR(l_insight, '.', -1) > 80 THEN
          l_insight := SUBSTR(l_insight, 1, INSTR(l_insight, '.', -1));
        END IF;
        l_insight_src := 'AI';
...
.ef{
  ...
  /* Top-edge inner highlight. Bright on light surfaces, barely-there
     on dark ones - the cue that separates a card from a flat panel. */
  --edge:rgba(255,255,255,.9);
  /* Deeper than before: the canvas is no longer near-white, so cards
     need a stronger lift to separate from it. */
  --lift:0 1px 2px rgba(16,24,40,.05), 0 14px 36px -16px rgba(16,24,40,.22);
  --lift2:0 2px 6px rgba(16,24,40,.07), 0 28px 56px -22px rgba(16,24,40,.30);
  --track:rgba(0,0,0,.09);
...
/* ---------- canvas ----------
   The page background was #f5f5f7 and the cards #ffffff - about 1.5%
   apart in luminance, so nothing read as a panel and the whole screen
   looked like one flat white sheet. The canvas is now deeper and
   faintly tinted, which is what lets a white card actually lift off it.

   Two soft washes sit over that gradient. They are ambient depth, not
   decoration: very large, very low opacity, anchored off the top
   corners so they never draw attention to themselves.

   custom.css pins the body background with !important, so this has to
   match. It only applies on the dashboard, because this stylesheet
   only exists inside the dashboard region. */
html body.t-PageBody{
  background:
    radial-gradient(1100px 620px at 12% -12%, rgba(11,99,206,.07), transparent 62%),
    radial-gradient(900px 520px at 96% -6%,  rgba(0,170,160,.06), transparent 62%),
    linear-gradient(180deg, #e9edf4 0%, #f4f6fa 45%, #f7f8fb 100%) !important;
  background-attachment:fixed !important;
}
html[data-theme="dark"] body.t-PageBody{
  background:
    radial-gradient(1100px 620px at 12% -12%, rgba(41,151,255,.13), transparent 62%),
    radial-gradient(900px 520px at 96% -6%,  rgba(48,209,88,.07), transparent 62%),
    linear-gradient(180deg, #0c1016 0%, #07090d 55%, #05070a 100%) !important;
  background-attachment:fixed !important;
}
...
/* ---------- hero ----------
   Three deliberate materials, so depth carries the hierarchy instead
   of more boxes: the brief is a tinted inset (information), the
   recommendation is a raised surface (action), the search field is
   sunken (utility). Everything else here is either meaningful or a
   physical cue - no ornament for its own sake.

   --dial1 / --dial2 are the health band colours, set inline on .ef by
   the renderer, so the card's accents follow the score. */
/* ---------- hero: the command surface ----------
   A page of white cards on a near-white canvas has no focal point, so
   the hero is now a dark surface and everything below stays light.
   Dark-hero-over-light-body is a long-standing pattern (Apple product
   pages, Stripe) and it gives the page one obvious place to look.

   The trick that keeps this maintainable: rather than restyling every
   child, the hero REDEFINES the theme tokens for its own subtree. All
   descendants already consume --ink, --muted, --card, --sunk, so they
   invert automatically and stay correct in both themes. */
.hero{position:relative;overflow:hidden;
  --ink:#f7f8fa; --ink2:#c9d2de; --muted:#95a1b2; --faint:#77828f;
  --card:#171d27; --sunk:rgba(255,255,255,.055);
  --line:rgba(255,255,255,.10); --hair:rgba(255,255,255,.09);
  --edge:rgba(255,255,255,.10); --track:rgba(255,255,255,.14);
  color:var(--ink);
  /* The deep blue from the original design. Two soft glows sit over
     it - kept large, low-opacity and anchored off the corners so they
     read as depth rather than decoration. */
  background:
    radial-gradient(520px 360px at 92% -18%, rgba(66,165,255,.30), transparent 68%),
    radial-gradient(460px 320px at 18% 118%, rgba(0,113,227,.22), transparent 70%),
    linear-gradient(135deg, #0b0f19 0%, #14213d 55%, #0b1a33 100%);
  border:1px solid rgba(255,255,255,.09);border-radius:26px;
  box-shadow:0 26px 64px -26px rgba(6,12,28,.62), inset 0 1px 0 rgba(255,255,255,.10);
  padding:44px 48px;margin-bottom:22px;
  display:grid;grid-template-columns:minmax(0,1fr) auto;gap:56px;align-items:center;}
/* Health as a hairline across the top edge - the state of the
   workspace is legible before a single word is read. */
.hero::before{content:"";position:absolute;left:0;right:0;top:0;height:3px;z-index:2;
  background:linear-gradient(90deg,var(--dial1),var(--dial2) 38%,transparent 88%);}
...
.kick .dot{width:7px;height:7px;border-radius:50%;background:var(--good);flex:none;
  animation:ef-pulse 2.8s ease-in-out infinite;}
@keyframes ef-pulse{
...
/* Raised, not sunken: this is the one thing on the card you are meant
   to act on, so it sits above the surface the brief is inset into. */
.rec{display:flex;gap:32px;align-items:flex-start;flex-wrap:wrap;
  margin-top:16px;padding:22px 26px;border-radius:16px;
...
/* ---------- score ----------
   No ring, no arc, no bar. Any closed or filling shape reads as a
   battery or a progress meter, and a value already expressed out of
   100 does not need a second, redundant depiction of the same
   proportion. It is set as a display figure instead - the way a
   headline statistic is set on a product page.

   A soft halo behind it gives the number presence without drawing a
   container around it. */
.score{position:relative;flex:none;text-align:right;padding:6px 4px 6px 0;}
.score::before{content:"";position:absolute;right:-30px;top:50%;
  width:340px;height:340px;margin-top:-170px;border-radius:50%;
  pointer-events:none;z-index:0;
  background:radial-gradient(circle,
    color-mix(in srgb,var(--dial2) 20%,transparent), transparent 64%);}
.score > *{position:relative;z-index:1;}
.score-n{font-size:112px;font-weight:600;letter-spacing:-.055em;line-height:.86;
  color:#fff;font-variant-numeric:tabular-nums;}
.score-l{display:flex;align-items:center;justify-content:flex-end;gap:9px;
  margin-top:10px;font-size:15px;font-weight:600;letter-spacing:.09em;}
.score-l i{width:9px;height:9px;border-radius:50%;background:var(--dial2);flex:none;
  box-shadow:0 0 12px 1px color-mix(in srgb,var(--dial2) 65%,transparent);}
.score-c{margin-top:12px;font-size:15px;color:var(--muted);letter-spacing:-.014em;}
...
/* ---- tablet: hero stacks, score left-aligns under the content ---- */
@media (max-width:880px){
  .hero{grid-template-columns:1fr;gap:30px;padding:36px 32px;}
  .hero h1{font-size:42px;}
  /* The size lives on .brief p, not .brief - overriding the wrapper
     did nothing, which is why the brief stayed at 22px on phones. */
  .brief p{font-size:19px;}
  .score{text-align:left;}
  .score-l{justify-content:flex-start;}
  .score-n{font-size:88px;}
}

/* ---- phone ---- */
@media (max-width:620px){
  .cd{padding:22px 18px;border-radius:18px;margin-bottom:16px;}
  .hero{padding:26px 20px;border-radius:20px;gap:22px;}
  .hero h1{font-size:31px;margin-bottom:16px;}
  .kick{font-size:14px;gap:8px;margin-bottom:12px;}
  .brief{padding:16px 18px 16px 21px;}
  .brief p{font-size:17px;}
  .brief-l{font-size:11px;margin-bottom:7px;}

  /* Score becomes a single compact line rather than a stacked block,
     so it reads as a summary instead of eating half the screen. */
  .score{display:flex;align-items:baseline;flex-wrap:wrap;gap:6px 14px;padding:0;}
  .score::before{display:none;}          /* halo overflows a narrow card */
  .score-n{font-size:54px;line-height:1;}
  .score-l{margin-top:0;font-size:13px;}
  .score-c{margin-top:0;width:100%;font-size:14px;}

  .rec{flex-direction:column;gap:16px;padding:18px 20px;}
  .rec-r{min-width:0;}
  .rec-t{font-size:18px;}
  .rec-w{font-size:15px;}
  .omni{padding:12px 15px;margin-top:18px;}
  .omni span{font-size:16px;}

  .hd h2{font-size:21px;}
  .hd .note{font-size:14px;}

  /* Let long task names wrap instead of being chopped to an ellipsis -
     on a phone the truncated version is often unreadable. */
  .act{gap:14px;padding:16px 4px;align-items:flex-start;}
  .act .t{font-size:17px;white-space:normal;}
  .act .m{white-space:normal;}
  .go{padding:9px 14px;font-size:15px;align-self:center;}
  .rank{margin-top:2px;}

  .row{gap:12px;padding:13px 4px;}
  .nm{font-size:17px;}
  .sub{font-size:14px;}
  .dt{width:54px;font-size:14px;}
  .rt{font-size:14px;}
  ...
  .ws .l{font-size:16px;}
  .ws .n{font-size:18px;}
  .pl{height:48px;gap:4px;}
  .pd{gap:4px;}
  .pd span{font-size:11px;}
}
...
@media (prefers-reduced-motion:reduce){
  ...
  -- HERO
  --------------------------------------------------------------------
  -- The health band colours are published as custom properties so the
  -- hero's top rule, wash and halo all track the score without any of
  -- them needing to know how it was calculated.
  p('<div class="ef" style="--dial1:'||l_dial_c1||';--dial2:'||l_dial_c2||';">');
  p('<div class="hero"><div>');
  ...
  -- Gate on there being an action at all, not on the delta clearing a
  -- whole point. Below one point the before/after are shown to one
  -- decimal so the arrow never reads "90 -> 90"; at or above a point
  -- they stay whole numbers.
  IF l_rec_kind IS NOT NULL AND l_rec_delta > 0.05 THEN
    DECLARE
      l_from VARCHAR2(20);
      l_to   VARCHAR2(20);
      l_amt  VARCHAR2(20);
    BEGIN
      IF l_rec_delta >= 0.95 THEN
        l_from := TO_CHAR(l_health);
        l_to   := TO_CHAR(l_rec_after);
        l_amt  := TO_CHAR(ROUND(l_rec_delta));
      ELSE
        l_from := dec1(l_health_raw);
        l_to   := dec1(l_rec_after_raw);
        l_amt  := dec1(l_rec_delta);
      END IF;
      ...
      p('<div class="rec-r"><div class="rec-l">EXPECTED IMPACT</div>'
        ||'<div class="rec-n"><span class="f">'||l_from||'</span>'
        ||'<span class="a">&rarr;</span><span class="t">'||l_to||'</span>'
        ||'<span class="d">+'||l_amt||'</span></div>'
  ...
  -- The score as a display figure. No ring, arc or bar: see .score in
  -- the stylesheet for why.
  p('<div class="score" role="img" aria-label="Delivery health '||l_health
    ||' out of 100, '||LOWER(l_health_label)||'">');
  p('<div class="score-n">'||l_health||'</div>');
  p('<div class="score-l" style="color:'||l_dial_c1||';"><i></i>'
    ||UPPER(l_health_label)||'</div>');
  p('<div class="score-c">'||l_tasks_open||' open &middot; '||l_task_pct||'% done</div>');
  p('</div>');
