-- ============================================================
-- EnterpriseFlow - Dashboard renderer, v7
--
...
CREATE OR REPLACE FUNCTION "RENDER_DASHBOARD_HTML" RETURN CLOB IS
  ...
  c_circ CONSTANT PLS_INTEGER := 383;      -- 2*pi*61, integer for NLS safety
  ...
  PROCEDURE add_action(p_title VARCHAR2, p_meta VARCHAR2, p_cta VARCHAR2,
                       p_href VARCHAR2, p_sev VARCHAR2) IS
  BEGIN
...
BEGIN
      ...
      FROM (SELECT pj.project_id, pj.project_name,
              ...
              CROSS APPLY (
                SELECT COUNT(CASE WHEN ts.status_code NOT IN ('DONE','CANCELLED')
                 ...
                 WHERE t.project_id = pj.project_id) c
  ...
  l_fingerprint := l_projects||'.'||l_projects_active||'.'||l_projects_done||'.'||l_tasks||'.'
  ...
  BEGIN
    ...
    IF NOT l_fresh THEN
      l_prompt :=
        ...
        ||CASE WHEN l_worst_overdue_name IS NOT NULL
               THEN ' Worst overdue: "'||l_worst_overdue_name||'", '||l_worst_overdue_days||' days late.' END
        ||CASE WHEN l_risk_proj_name IS NOT NULL
               THEN ' Riskiest project: "'||l_risk_proj_name||'" ('||l_risk_proj_why||').' END
        ||' You are briefing an executive. Write exactly two short sentences, 30 words total maximum.'
        ||' First: the single most important thing to act on and why it matters.'
        ||' Second: one line of overall context. Plain text only, no markdown, no greeting.';
...
/* ---------- hero ---------- */
.hero{padding:10px 4px 34px;margin-bottom:22px;
  display:grid;grid-template-columns:minmax(0,1fr) auto;gap:48px;align-items:start;}
.kick{display:flex;align-items:center;gap:11px;font-size:16px;color:var(--muted);margin-bottom:18px;}
.kick .dot{width:7px;height:7px;border-radius:50%;background:var(--good);flex:none;}
.kick .sep{color:var(--track);}
.hero h1{font-size:52px;font-weight:600;letter-spacing:-.03em;line-height:1.06;margin:0 0 16px;}
.brief{font-size:21px;font-weight:400;line-height:1.42;letter-spacing:-.019em;
  color:var(--ink2);margin:0;max-width:46ch;}

/* search / ask field - the Ctrl-K entry point */
.omni{display:flex;align-items:center;gap:12px;margin-top:26px;max-width:520px;
  background:var(--card);border:1px solid var(--hair);box-shadow:var(--lift);
  border-radius:14px;padding:14px 18px;cursor:text;transition:box-shadow .18s,transform .18s;}
.omni:hover{box-shadow:var(--lift2);transform:translateY(-1px);}
...
.omni kbd{font-family:inherit;font-size:13px;font-weight:600;color:var(--muted);
  background:var(--sunk);border-radius:6px;padding:4px 8px;letter-spacing:0;}

/* health dial */
.dial{position:relative;width:172px;height:172px;flex:none;}
.dial svg.r{transform:rotate(-90deg);display:block;width:172px;height:172px;}
.dial .tr{fill:none;stroke:var(--track);stroke-width:5;}
.dial .vl{fill:none;stroke:var(--accent);stroke-width:5;stroke-linecap:round;}
.dial .ct{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;}
.dial .n{font-size:60px;font-weight:600;letter-spacing:-.035em;line-height:1;}
.dial .l{font-size:14px;font-weight:600;letter-spacing:.01em;color:var(--muted);margin-top:8px;}
.dial .c{font-size:13px;color:var(--faint);margin-top:3px;}
.dial.w .vl{stroke:var(--warn);}
.dial.c2 .vl{stroke:var(--late);}
...
.act .t{display:block;font-size:21px;font-weight:500;letter-spacing:-.021em;
...
.pc .t{font-size:19px;font-weight:500;letter-spacing:-.021em;margin-bottom:5px;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
...
.sub{font-size:16px;color:var(--muted);letter-spacing:-.014em;display:block;margin-top:2px;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.dt{font-size:16px;font-weight:600;width:70px;flex:none;letter-spacing:-.014em;}
...
@keyframes rg{from{stroke-dashoffset:383;}}
...
@media (prefers-reduced-motion:reduce){
  ...
  p('<h1>'||l_greeting||'</h1>');
  p('<p class="brief">'||esc(l_insight)||'</p>');
  ...
  p('<div class="dial'||CASE WHEN l_health < 60 THEN ' c2' WHEN l_health < 75 THEN ' w' END
    ||'" role="img" aria-label="Delivery health '||l_health||' out of 100, '||LOWER(l_health_label)||'">');
  p('<svg class="r" viewBox="0 0 140 140" aria-hidden="true">'
    ||'<circle class="tr" cx="70" cy="70" r="61"/>'
    ||'<circle class="vl" cx="70" cy="70" r="61" stroke-dasharray="'||TO_CHAR(c_circ)
    ||'" stroke-dashoffset="'||TO_CHAR(l_ring_off)||'"/></svg>');
  p('<div class="ct"><span class="n">'||l_health||'</span><span class="l">'||UPPER(l_health_label)
    ||'</span><span class="c">'||l_tasks_open||' open &middot; '||l_task_pct||'% done</span></div></div>');
        ...
        CROSS APPLY (
          SELECT COUNT(*) tot,
                 ...
                 COUNT(CASE WHEN ts.status_code NOT IN ('DONE','CANCELLED')
                             AND t.assigned_to_employee_id IS NULL THEN 1 END) un
            FROM task t JOIN status ts ON ts.status_id = t.status_id
           WHERE t.project_id = pj.project_id) cnt
       WHERE s.status_code NOT IN ('COMPLETED','CANCELLED')
       ORDER BY risk DESC, pj.project_name
  ...
  DECLARE
    ...
    BEGIN
      IF LENGTH(l_j) > 28000 THEN RETURN; END IF;   -- hard cap, stay inside VARCHAR2
      IF l_n > 0 THEN l_j := l_j || ','; END IF;
      l_n := l_n + 1;
      l_j := l_j || '{"k":"'||p_k||'","n":"'||js(p_n)||'","s":"'||js(p_s)||'","u":"'||js(p_u)||'"}';
    END item;
  BEGIN
    ...
    FOR r IN (SELECT t.task_id, t.task_name, pj.project_name
               ...
               ORDER BY t.task_name FETCH FIRST 60 ROWS ONLY) LOOP
    ...
    FOR r IN (SELECT e.employee_id, e.first_name||' '||e.last_name nm, rl.role_name
               ...
               WHERE e.active_flag = 'Y'
               ORDER BY e.last_name FETCH FIRST 60 ROWS ONLY) LOOP
      item('Person', r.nm, NVL(r.role_name,'-'),
           APEX_PAGE.GET_URL(p_page => 14, p_items => 'P14_EMPLOYEE_ID',
                             p_values => TO_CHAR(r.employee_id)));
    END LOOP;

    FOR r IN (SELECT c.client_id, c.client_name FROM client c ORDER BY c.client_name) LOOP
