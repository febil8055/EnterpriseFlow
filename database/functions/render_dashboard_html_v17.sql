-- ============================================================
-- EnterpriseFlow - Dashboard renderer, v17
--
...
CREATE OR REPLACE FUNCTION "RENDER_DASHBOARD_HTML" RETURN CLOB IS
  ...
  -- Gauge geometry. A closed ring at 90% reads as a battery or a
  -- loading spinner no matter how it is styled, so the score is drawn
  -- as an open 270-degree arc with a gap at the bottom and a marker at
  -- the value - the grammar of an instrument, not a cell.
  l_arc_len NUMBER;              -- length of arc to draw, in user units
  l_mark_x  NUMBER;              -- marker position on the arc
  l_mark_y  NUMBER;
  l_dial_c1 VARCHAR2(10); l_dial_c2 VARCHAR2(10);   -- gradient stops, banded
  -- r=78 in a 200x200 viewBox. Full circumference 490; the gauge uses
  -- 270 degrees of it = 368. Integers throughout so no SVG attribute
  -- ever depends on the session's decimal separator.
  c_arc    CONSTANT PLS_INTEGER := 368;
  c_radius CONSTANT PLS_INTEGER := 78;
...
BEGIN
  ...
  -- Arc starts at 135 degrees (lower left) and sweeps 270 clockwise,
  -- leaving a 90 degree gap at the bottom for the label to sit in.
  l_arc_len := ROUND(c_arc * l_health / 100);
  DECLARE
    l_ang NUMBER := (135 + 270 * l_health / 100) * ACOS(-1) / 180;
  BEGIN
    l_mark_x := ROUND(100 + c_radius * COS(l_ang));
    l_mark_y := ROUND(100 + c_radius * SIN(l_ang));
  END;
...
.hero{position:relative;overflow:hidden;
  ...
  /* inset highlight along the top edge: the cue that makes a surface
     read as a physical card rather than a coloured rectangle */
  box-shadow:var(--lift), inset 0 1px 0 var(--edge);
  padding:40px 44px;margin-bottom:22px;
...
/* A wash of the band colour behind the dial. Clipped by the card, so
   it grounds the score rather than floating behind the whole page. */
.hero::after{content:"";position:absolute;right:-70px;top:-90px;
  ...
  background:radial-gradient(circle,
    color-mix(in srgb,var(--dial2) 13%,transparent), transparent 68%);}
...
/* Soft halo in the band colour. Sits behind the ring, so the score
   reads as lit rather than pasted on. */
.dial::before{content:"";position:absolute;inset:-18px;border-radius:50%;z-index:0;
  pointer-events:none;
  background:radial-gradient(circle,
    color-mix(in srgb,var(--dial2) 16%,transparent), transparent 66%);}
/* No CSS rotation here - the arc group carries its own rotate(135),
   and rotating the svg as well would move the gap off the bottom. */
.dial svg.r{position:relative;z-index:1;display:block;width:196px;height:196px;}
.dial .tr{fill:none;stroke:var(--track);stroke-width:12;stroke-linecap:round;opacity:.55;}
.dial .vl{fill:none;stroke-width:12;stroke-linecap:round;}
.dial .mkbg{fill:var(--card);}
.dial .mk{fill:var(--dial2);}
.dial .ct{z-index:2;position:absolute;inset:0;display:flex;flex-direction:column;
...
/* Sweeps the arc out from zero. Only a `from` is given, so `both`
   settles on the element's own inline dasharray - no constant to keep
   in sync with the PL/SQL. */
@keyframes rg{from{stroke-dasharray:0 999;}}
...
@media (prefers-reduced-motion:reduce){
  ...
  p('<svg class="r" viewBox="0 0 200 200" aria-hidden="true">'
    ...
    -- Rotated as a group so both arcs share one origin, and the gap
    -- lands centred at the bottom rather than wherever SVG starts.
    ||'<g transform="rotate(135 100 100)">'
    ||'<circle class="tr" cx="100" cy="100" r="'||TO_CHAR(c_radius)
    ||'" stroke-dasharray="'||TO_CHAR(c_arc)||' 999"/>'
    ||'<circle class="vl" cx="100" cy="100" r="'||TO_CHAR(c_radius)
    ||'" stroke="url(#efDialG)" stroke-dasharray="'||TO_CHAR(l_arc_len)||' 999"/>'
    ||'</g>'
    -- Marker at the reading. The outer disc is punched in the card
    -- colour so the dot sits cleanly on the arc instead of smudging it.
    ||'<circle class="mkbg" cx="'||TO_CHAR(l_mark_x)||'" cy="'||TO_CHAR(l_mark_y)||'" r="9"/>'
    ||'<circle class="mk" cx="'||TO_CHAR(l_mark_x)||'" cy="'||TO_CHAR(l_mark_y)||'" r="5"/>'
    ||'</svg>');
